...
class F2PP2PAnalyzer:
    ...
    def _generate_test_patch(self, base_sha: str, head_sha: str, test_files: List[str]) -> str:
        """Generate a patch for selected test file paths (SWE-Bench-style).
    ...
    def _apply_test_patch_from_head(self, test_files: List[str], base_sha: str, head_sha: str):
        """Apply a SWE-Bench-style test patch onto the current working tree (expected to be at base_sha)."""
        if not test_files:
            return
    ...
    def _generate_full_patch(self, base_sha: str, head_sha: str) -> str:
        """Generate a full patch (tests + source) transforming base -> head (SWE-Bench-style)."""
        try:
            result = subprocess.run(
    ...
    def _apply_full_patch_from_head(self, base_sha: str, head_sha: str):
        """Apply a SWE-Bench-style full patch onto the current working tree (expected to be at base_sha)."""
    ...
    def _run_at_commit(
        ...
        if apply_full_patch:
            ...
            # SWE-Bench semantics for BEFORE: checkout base, then apply a patch of test changes.
